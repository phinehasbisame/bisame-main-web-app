...
export async function GET(request: NextRequest) {
  try {
    const apiUrl = process.env.NEXT_PUBLIC_POSTFORM_LAPTOP_COMPUTER_API_URL;
    if (!apiUrl) {
      return NextResponse.json(
        { error: 'Laptop Computer API URL not configured', code: 'CONFIG_ERROR' }, 
        { status: 500 }
      );
    }
    ...
    console.error('Laptop Computer API error:', error);
    ...
    if (error instanceof AxiosError) {
      ...
      if (error.code === 'ENOTFOUND' || error.code === 'ECONNREFUSED') {
        return NextResponse.json(
          { error: 'Laptop Computer service unavailable', code: 'SERVICE_UNAVAILABLE' }, 
          { status: 503 }
        );
      }
