...
export async function GET(request: NextRequest) {
  try {
    const apiUrl = process.env.NEXT_PUBLIC_POSTFORM_WEDDING_WEAR_API_URL;
    if (!apiUrl) {
      return NextResponse.json(
        { error: 'Wedding Wear API URL not configured', code: 'CONFIG_ERROR' }, 
        { status: 500 }
      );
    }
    ...
    console.error('Wedding Wear API error:', error);
    ...
    if (error instanceof AxiosError) {
      ...
      if (error.code === 'ENOTFOUND' || error.code === 'ECONNREFUSED') {
        return NextResponse.json(
          { error: 'Wedding Wear service unavailable', code: 'SERVICE_UNAVAILABLE' }, 
          { status: 503 }
        );
      }
