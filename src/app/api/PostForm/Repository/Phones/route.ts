...
export async function GET(request: NextRequest) {
  try {
    const apiUrl = process.env.NEXT_PUBLIC_POSTFORM_MOBILE_PHONE_API_URL;
    if (!apiUrl) {
      return NextResponse.json(
        { error: 'Mobile Phone API URL not configured', code: 'CONFIG_ERROR' },
        { status: 500 }
      );
    }
    ...
    console.error('Mobile Phone API error:', error);
    ...
    if (error instanceof AxiosError) {
      ...
      if (error.code === 'ENOTFOUND' || error.code === 'ECONNREFUSED') {
        return NextResponse.json(
          { error: 'Mobile phone service unavailable', code: 'SERVICE_UNAVAILABLE' },
          { status: 503 }
        );
      }
