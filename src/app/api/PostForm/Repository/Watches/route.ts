...
export async function GET(request: NextRequest) {
  try {
    const apiUrl = process.env.NEXT_PUBLIC_POSTFORM_WATCHES_API_URL;
    if (!apiUrl) {
      return NextResponse.json(
        { error: 'Watches API URL not configured', code: 'CONFIG_ERROR' }, 
        { status: 500 }
      );
    }
    ...
    if (error instanceof AxiosError) {
      ...
      if (error.code === 'ENOTFOUND' || error.code === 'ECONNREFUSED') {
        return NextResponse.json(
          { error: 'Watches service unavailable', code: 'SERVICE_UNAVAILABLE' }, 
          { status: 503 }
        );
      }
