...
export async function GET(request: Request) {
  try {
    const apiUrl = process.env.NEXT_PUBLIC_MY_FOLLOWING_API_URL;
    const baseUrl = process.env.NEXT_PUBLIC_LISTINGS_BASE_URL;
    
    if (!apiUrl) {
      return NextResponse.json(
        { success: false, message: 'Following API URL not configured.' },
        { status: 500 }
      );
    }
    ...
    const response = await axios.get<FollowingApiResponse>(fullUrl, {
      headers: {
    ...
    if (axios.isAxiosError(error)) {
      ...
      return NextResponse.json(
        {
          success: false,
          message: error.response?.data?.message || 'Failed to fetch following data.',
    ...
    console.error('Server error in Following route:', error);
    return NextResponse.json(
